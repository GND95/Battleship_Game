...
namespace Battleship
{
    public partial class Form1 : Form
    {
        ...
        class Shell // Munition, Torpedo, Bomb, Projectile, Shot, Round, Ordnance, Ammunition, Weapon, Rocket, Missle
        {
            ...
            double x, y; // impact location in global cartesian coordinates
            int damage;
        ...
        class Gun
        {
            ...
            float velocity; // in cartesian units
            int loadtime;
        ...
        class Ship
        {
            string name;
            int hp;
            bool ff; //Friend or Foe - true = Friend
            Gun gun;
            float x;
            float y;
            float h;
            ...
            int engine; // 0 = full stop, 1 = ahead 1 quarter, 2, 3, 4, = ahead full 
            Bitmap original;
            ...
            public float Heading //Read Only
            {
                get { return h; }
                //set { h = value; }
            }
            ...
            public void Move()
            {
                ...
                if (h != dh)
                {
                    if (h < dh)
                    {
                        h += (float)Math.Min(0.2, dh - h);
                    }
                    else // h>dh
                    {
                        h -= (float)Math.Min(0.2, h - dh);
                    }
                }
                ...
                if (s != ds)
                {
                    if (ds < s) //slow down
                    {
                        s -= (float)Math.Min(0.001F, s - ds);
                    }
                    else // speed up
                    {
                        s += (float)Math.Min(0.001F, ds - s);
                    }
                }
        ...
        float MinimapWidth; // in cartesian units
        //float MinimapHeight; // in cartesian units
        Color MinimapColor;
        ...
        public Form1()
        {
            ...
            HeadsUpX = 100;
            HeadsUpY = 100;
            HeadsUpColor = Color.Beige;
            Ships = new System.Collections.ArrayList();
            Ships.Add(new Ship("Missouri", 0, 0, 75, 0, "missouri-s1.bmp",true));
            Ships.Add(new Ship("Iowa", 0, 150, 90, .75F, "missouri-s1.bmp",true));
            Ships.Add(new Ship("Enemy 1", 2500, 2500, 215, 1.0F, "hood-s1.bmp", false));
            Ships.Add(new Ship("Enemy 1", 2500, 2750, 215, 1.0F, "hood-s1.bmp", false));
            SelectedShipIndex = 0;
        ...
        private void DrawView()
        {
            ...
            //Draw a red dot for shell impact point
            ...
            g.DrawString(SelectedShip.Heading.ToString("f1"), font, brush, new PointF(HeadsUpX, HeadsUpY + 25));
            ...
            foreach (Ship s in Ships)
            {
                ...
                if (s.FriendOrFoe == true)
                {
                    ...
                    gm.FillRectangle(new SolidBrush(MinimapEnemyShipColor), x, y, 2, 2);
            
            }
            //Draw view frame on Minimap
            ...
            gm.DrawRectangle(new Pen(Color.Red), x1, y1, xw, yw);
            // Draw Minimap onto View bitmap
            ...
            {
                case 1:
                    g.DrawEllipse(new Pen(Color.FromArgb(100 - Hourglass * 10, Color.Red), 3), SelectedShip.X - ViewX - 125 + pictureBox1.Width / 2, ViewY - SelectedShip.Y - 125 + pictureBox1.Height / 2, 250, 250);
                    break;
                case 2:
                    g.DrawEllipse(new Pen(Color.FromArgb(Hourglass * 10, Color.Red), 3), SelectedShip.X - ViewX - 125 + pictureBox1.Width / 2, ViewY - SelectedShip.Y + pictureBox1.Height / 2 - 125, 250, 250);
                    break;
            }
            ...
            foreach (Shell s in Shells)
            {
                if (Clock >= s.Time) // hits impact point
                {
                    ...
                    foreach (Ship ship in Ships)
                    {
                        ...
                        {
                            ship.HP -= s.Damage;
                            Explosions.Add(new Explosion(s.X, s.Y, 50));
                            Miss = false;
                        }
                    }
                    DeadShells.Add(s);
        ...
        private void GoToNextShip()
        {
            ...
            do
            {
                if (PossibleIndex < Ships.Count - 1) //not at last ship in collection
                {
                    ++PossibleIndex;
                }
                else //is at last ship in collection
                {
                    PossibleIndex = 0;
                }
